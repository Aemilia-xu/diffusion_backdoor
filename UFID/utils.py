...
def get_sd_model(method='',benign_path='/data_19/pretrained_model/stability/stable-diffusion-v1-4',backdoor_path=None):
...
def get_prompt(method, trigger, replace_word, num=300):
    if method == 'Rickrolling':
        prompt_file = '/data_19/backdoor/backdoor_attack/AntProject/resource/captions_10000_o.txt'
        ...
        prompt_file = '/data_19/backdoor/backdoor_attack/AntProject/resource/captions_10000.txt'
        with open(prompt_file, 'r') as f:
            lines = f.readlines()
        if method == 'EvilEdit':
...
# pipe = get_sd_model(benign_path='/data_19/pretrained_model/stability/stable-diffusion-v1-5',unet_path=unet_path)
